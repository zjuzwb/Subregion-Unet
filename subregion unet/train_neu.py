...
def train(cfg, writer, logger):
    ...
    val_transforms = Compose([TestRescale(input_hw=(192, 192)),
                               ToTensor(),  # /255
                               # Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
                               ])

    train_loader = DataGenerator(txtpath='./dataset/trainneu.txt', transformer=train_transforms)
    n_classes =4
    ...
    val_loader = DataGenerator(txtpath='/dataset/trainneu.txt', transformer=val_transforms)

    valloader = data.DataLoader(
        val_loader, batch_size=cfg["training"]["batch_size"], num_workers=cfg["training"]["n_workers"], drop_last=True, shuffle=True
    )
    ...
    while i <= cfg["training"]["train_iters"] and flag:
        for (images, labels,label1) in trainloader:
            ...
            output_d = outputs[:, 1:, :, :].sum(dim=1)
            output_f = outputs[:, 0, :, :]
            maskout = torch.stack([output_f, output_d], dim=1)
            ...
            if (i + 1) % cfg["training"]["val_interval"] == 0 or (i + 1) == cfg["training"][
                ...
                if score["Mean IoU : \t"] >= best_iou:
                    ...
                    save_path = os.path.join(
                        'model_savePath',
                                "{}_{}_best_newmodel_neu.pkl".format(cfg["model"]["arch"], cfg["data"]["dataset"]),
                    )
